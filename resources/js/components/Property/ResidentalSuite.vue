      ...
      <div class="form-card">
        ...
        <div v-if="list_showable" class="card-body">
          ...
          <vue3-datatable :rows="rows" :columns="columns" :sortable="true" class="advanced-table whitespace-nowrap">
            ...
            <template #suite_no="data">
              <span class="font-semibold">{{ data.value.suite_no }}</span>
            </template>
            ...
            <template #suite_type="data">
              <span class="font-semibold">{{ data.value.suite_type }}</span>
            </template>
            <template #total_suite_size="data">
              <span class="font-semibold">{{ data.value.total_suite_size }}</span>
            </template>
            <template #buttons="data">
              <button class="btn btn-primary btn-sm" @click.prevent="editCommercialUnit(data.value.id)">Edit</button>
              <button class="btn btn-danger btn-sm" @click.prevent="CommercialUnitDelete(data.value.id)">Delete</button>
            </template>
          </vue3-datatable>
        </div>
        <div v-if="!list_showable">
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-12 col-sm-12 col-xs-12">
                        ...
                        <select v-model="form.suite_no"
                          ...
                          <option v-for="suite in form.total_suite" :value="suite">Suite No
                            {{ form.floor_name_string }}-{{ suite.toString().padStart(2, "0") }}
                          </option>
                        </select>
                      </div>
                    ...
                    <div class="form-holder">
                      <div class="form-check-inline">
                        ...
                        <input type="radio"
                               ...
                               name="commercial"
                               v-model="form.commercial" disabled>
                        <label for="commercial">Sub Rooms Total Size: {{ form.total_subroom_size }}
                          {{ uom_arr[form.suite_uom] }}</label><br>

                        <input type="radio"
                               id="residential_commercial"
                               name="residential_commercial"
                               v-model="form.residential_commercial" disabled>
                        <label for="residential_commercial">Total Suite Size: {{ form.total_suite_size }}
                          {{ uom_arr[form.suite_uom] }}</label><br>
                      </div>
                    </div>
                  </div>
          ...
          <div class="form-folder">
            <h3><i class="fa fa-hand-point-right"></i>Residential Suites Details
                      ...
                      <template v-if="form.total_suite>0">
                        ...
                        <template v-for="(single_subroom,index) in form.subrooms_list_arr">
                          ...
                          <tr v-if="single_subroom.disable"
                              style="background-color:rgb(228, 233, 230); border-bottom:1px solid #fff;">
                            <td width="25%" scope="row" style="padding-left:5% !important">
          ...
          <div class="text-center" v-if="!list_showable">
            ...
            <button :disabled="form.busy || editmode==false || form.posting_status!=0" type="button"
                    class="btn  btn-primary" style="min-width:110px" @click.prevent="deleteCustomer()">Delete
            </button>
...
export default {
  ...
  data() {
    return {
      ...
      columns: ref([
        ...
        {title: 'Suite No', field: 'suite_no'},
        {title: 'Property ID', field: 'system_no'},
        {title: 'Property Name', field: 'property_name'},
        {title: 'Suite Type', field: 'suite_type'},
        {title: 'Total Suite Size', field: 'total_suite_size'},
  ...
  methods: {
    ...
    reset_list() {
      ...
      window.axios.get(uri).then((response) => {
        this.rows = ref(response.data.residential_suite_list);
      });
