<template>
  ...
  <fieldset>
    <form id="msform" @submit.prevent="editmode ? updateStoragelot() : createCompanyProfile()"
        ...
        <div v-if="list_showable" class="card-body">
          ...
          <vue3-datatable :rows="rows" :columns="columns" :sortable="true" class="advanced-table whitespace-nowrap">
            ...
            <template #lot_number="data">
              <span class="font-semibold">{{ data.value.lot_number }}</span>
            </template>
            <template #storage_lot_size_qty="data">
              <span class="font-semibold">{{ data.value.storage_lot_size_qty }}</span>
            </template>
            <template #buttons="data">
              <button class="btn btn-primary btn-sm" @click.prevent="editStorageLot(data.value.id)">Edit</button>
              <button class="btn btn-danger btn-sm" @click.prevent="StorageLotDelete(data.value.id)">Delete</button>
            </template>
          </vue3-datatable>
        </div>
        <div v-if="!list_showable">
                  ...
                  <div class="form-box-outer">
                    ...
                    <div class="form-holder">
                      <div class="form-check-inline">
                        ...
                        <label for="storage_lot_size_qty">Storage Lot Size:{{ form.storage_lot_size_qty }}
                          {{ uom_arr[form.lot_uom] }}</label><br>
                        <input type="radio"
                               id="storage_level_qty"
                               name="storage_level_qty" disabled checked>
                        <label for="storage_level_qty">Total Storage Level Size: {{ form.total_storage_level_size }}
                          {{ uom_arr[form.lot_uom] }}</label><br>
                        <input type="radio"
                               id="storage_locker_qty"
                               name="storage_locker_qty" disabled checked>
                        <label for="storage_locker_qty">Total Storage `Locker Size: {{ form.total_storage_locker_size }}
                          {{ uom_arr[form.lot_uom] }}</label><br>
                      </div>
                    </div>
          ...
          <div class="form-folder">
            <h3><i class="fa fa-hand-point-right"></i>Storage Lot Details
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_property()"
                      v-show="!property_show">Show
              </button>
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_property()" v-show="property_show">
                Hide
              </button>
            </h3>
                      ...
                      <template v-if="form.total_storage_lot>0">
                        <template v-for="(single_level,level) in form.subrooms_list_arr">
                          ...
                          <tr style="background-color: #6090bc;color: #fff;">
                            <td width="10%" scope="row" style=""><strong>Storage Level
                              {{ level.toString().padStart(2, "0") }}</strong></td>
                            <td width="150">
                          ...
                          <template v-for="(single_locker,lkr) in form.subrooms_list_arr[level]">
                            <tr v-if="lkr>=0">
                              <td width="10%" scope="row" style="padding-left:1% !important">
                                <strong>{{ single_locker.item_name }}</strong></td>
                              <td width="">
                              ...
                              <td>
                                <button type="button" class="btn btn-primary btn-sm" style="line-height:.7 !important"
                                        @click="add_locker_details(single_locker.details,level,lkr)">Details
                                </button>
          ...
          <div class="form-folder">
            <h3><i class="fa fa-hand-point-right"></i> Property Management Type
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_property_type()"
                      v-show="!property_type_show">Show
              </button>
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_property_type()"
                      v-show="property_type_show">Hide
              </button>
            </h3>
          ...
          <div class="form-folder">
            <h3><i class="fa fa-hand-point-right"></i> License and Permits
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_licency_permit()"
                      v-show="!licence_permit_show">Show
              </button>
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_licency_permit()"
                      v-show="licence_permit_show">Hide
              </button>
            </h3>
                  ...
                  <div class="form-box-outer">
                    <table class="table table_narrow">
                      ...
                      <template v-for="(license_and_permit,index) in form.license_and_permit_list_arr">
                        <tr style="background-color: #e7e8e7">
                          <td scope="row" width="18%" v-if="license_and_permit.reference_id">
                            {{ license_and_permit.item_name }}
                          </td>
                          <td v-else>
                            <input
                                type="text"
                      ...
                      <tr>
                        <td>
                          <button type="button" class="btn btn-primary btn-sm" @click="add_license_permit()">Add
                          </button>
                        </td>
                      </tr>
                      </tbody>
          ...
          <div class="form-folder">
            <h3><i class="fa fa-hand-point-right"></i>Safety Devices and Equipment's
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_safety_equipment()"
                      v-show="!safety_equipment_show">Show
              </button>
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_safety_equipment()"
                      v-show="safety_equipment_show">Hide
              </button>
            </h3>
                        ...
                        <tr style="background-color: #e7e8e7">
                          ...
                          <td>
                            <button type="button" class="btn btn-primary btn-sm"
                                    @click="change_fire_extinguisher(safety_item_list)">Add
                            </button>
                          </td>
                          <td></td>
                          <td></td>
                            ...
                            <tr>
                              ...
                              <td>
                                <date-picker
                                    ...
                                    name="renew_date[]"
                                    type="renew_date"
                                    :class="{ 'is-invalid': form.errors.has('renew_date') }"
                                />
                              </td>
                              <td>
                              ...
                              <td>
                                <date-picker
                                    ...
                                    name="due_on[]"
                                    type="due_on"
                                    :class="{ 'is-invalid': form.errors.has('due_on') }"
                                />
                              </td>
          ...
          <div class="form-folder">
            <h3><i class="fa fa-hand-point-right"></i>External Services Providers Accounts Details
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_service_provider()"
                      v-show="!external_service_show">Show
              </button>
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_service_provider()"
                      v-show="external_service_show">Hide
              </button>
            </h3>
                        ...
                        <tr style="background-color: #e7e8e7">
                          ...
                          <td>
                            <date-picker
                                ...
                                name="schedule_date[]"
                                type="due_on"
                                :class="{ 'is-invalid': form.errors.has('schedule_date') }"
                            />
                          </td>
                          <td>
                          ...
                          <td>
                            <date-picker
                                ...
                                type="due_on"
                                :class="{ 'is-invalid': form.errors.has('expected_due_date') }"
                            />
                          </td>
                          <td>
          ...
          <div class="form-folder">
            <h3><i class="fa fa-hand-point-right"></i> Contacts Details
              ...
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_contact_details()"
                      v-show="!contact_details_show">Show
              </button>
              <button type="button" class="btn btn-primary btn-sm" @click="show_hide_contact_details()"
                      v-show="contact_details_show">Hide
              </button>
            </h3>
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                  <div class="form-box-outer">
                    <table class="table table_narrow">
                      ...
                      <tbody style="border:none">
                      <template v-for="(category_contact_details,index) in form.building_contact_details_arr">
                        <template v-if="index>0">
                          ...
                          <tr>
                          <td>
                            <button type="button" class="btn btn-primary btn-sm" @click="add_contact_details(index)">
                              Add
                            </button>
                          </td>
          ...
          <div class="text-center" v-if="!list_showable">
            <button :disabled="form.busy || editmode==false" type="button" class="btn  btn-primary"
                    style="min-width:110px" @click="reset_form()">New
            </button>

            <button :disabled="form.busy || editmode==true || form.posting_status!=0" type="button"
                    class="btn  btn-primary" style="min-width:110px" @click="save_stay(1)">Save-Stay
            </button>
            <button :disabled="form.busy || editmode==true || form.posting_status!=0" type="submit"
                    class="btn  btn-primary" style="min-width:110px" @click="save_stay(2)">Save-Out
            </button>
            <button :disabled="form.busy || editmode==true || form.posting_status!=0" type="submit"
                    class="btn  btn-primary" style="min-width:110px" @click="save_stay(3)">Save-New
            </button>
            <button :disabled="form.busy || editmode==false || form.posting_status!=0" type="submit"
                    class="btn  btn-primary" style="min-width:110px">Update
            </button>
            <button :disabled="form.busy || editmode==false || form.posting_status!=0" type="button"
                    class="btn  btn-primary" style="min-width:110px" @click.prevent="deleteStorageLot()">Delete
            </button>


            <button :disabled="form.busy || editmode==false || form.posting_status>1" type="button"
                    class="btn  btn-primary" style="min-width:110px" @click="post()" v-if="user_type==9">Post
              {{ form.posting_status }}
            </button>
            <button :disabled="form.busy || editmode==false || form.posting_status!=0" type="button"
                    class="btn  btn-primary" style="min-width:110px" @click="post()" v-else>Post
            </button>

            <button :disabled="form.busy || form.posting_status<2 || user_type!=9" type="button"
                    class="btn  btn-primary" style="min-width:110px" @click="adjust()">Adjust
            </button>

            <button :disabled="form.busy || form.posting_status!=3 || user_type!=9" type="button"
                    class="btn  btn-primary" style="min-width:110px" @click="repost()">RePost
            </button>
            <button :disabled="form.busy" type="button" class="btn  btn-primary" style="min-width:110px"
      ...
      <!--  MOdel  -->
      <div class="modal fade model-middle" id="customContactModal" tabindex="-1" role="dialog"
           aria-labelledby="exampleModalLabel" z-index="1000" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="exampleModalLabel" v-if="form.custom_contact_type==1">Add Staff and
                Departments</h5>

              <h5 class="modal-title" id="exampleModalLabel" v-if="form.custom_contact_type==2">Add Repair and
                Maintenance</h5>

              <h5 class="modal-title" id="exampleModalLabel" v-else-if="form.custom_contact_type==3">Add Safety
                Equipment</h5>

              <h5 class="modal-title" id="exampleModalLabel" v-else-if="form.custom_contact_type==4">Add Dedicated
            ...
            <div class="modal-footer">
              ...
              <button type="button" class="btn btn-primary" @click.prevent="addNewCustomField()"
                      v-if="form.custom_contact_type==1">Add Staff and Departments
              </button>
              <button type="button" class="btn btn-primary" @click.prevent="addNewCustomField()"
                      v-if="form.custom_contact_type==2">Add Repair and Maintenance
              </button>
              <button type="button" class="btn btn-primary" @click.prevent="addNewCustomField()"
                      v-if="form.custom_contact_type==3">Add Safety Equipment
              </button>
              <button type="button" class="btn btn-primary" @click.prevent="addNewCustomField()"
                      v-if="form.custom_contact_type==4">Add Dedicated Service Providers
              </button>
      ...
      <!--  MOdel  -->
      <div class="modal fade model-middle" id="StorageLockerModal" tabindex="-1" role="dialog"
           aria-labelledby="exampleModalLabel" z-index="1000" aria-hidden="true">
        <div class="modal-dialog" role="document">
  ...
  data() {
    return {
      ...
      columns: ref([
        ...
        {title: 'Lot Size', field: 'storage_lot_size_qty'},
  ...
  methods: {
    ...
    editStorageLot(id) {
      ...
      window.axios.get(uri).then((response) => {
        ...
        this.form.license_and_permit_list_arr = response.data.license_and_permit_list_arr.map(item => {
          return {
            ...item,
            expiry_date: item.expiry_date && item.expiry_date !== "" ? new Date(item.expiry_date) : null
          };
        });
        ...
        this.form.strata_lot_no = response.data.storage_lot_arr.strata_lot_no;
        ...
        for (let i = 0; i < safety_device_equipment_arr.length; i++) {
          ...
          if (safety_device_equipment_arr[i].item_id == 1) {
            ...
            this.form.fire_extinguisher_details_arr.push({
              ...
              'expiry_date': safety_device_equipment_arr[i].expiry_date ? new Date(safety_device_equipment_arr[i].expiry_date) : null,
              'renew_date': safety_device_equipment_arr[i].renew_date ? new Date(safety_device_equipment_arr[i].renew_date) : null,
              'cicle': safety_device_equipment_arr[i].cicle,
              'due_on': safety_device_equipment_arr[i].due_on ? new Date(safety_device_equipment_arr[i].due_on) : null,
            });
        ...
        for (let i = 0; i < this.external_service_provider_list_arr.length; i++) {
          ...
          this.form.external_service_provider_details_arr.push({
            ...
            'schedule_date': this.external_service_provider_list_arr[i].schedule_date ? new Date(this.external_service_provider_list_arr[i].schedule_date) : null,
            'expected_due_date': this.external_service_provider_list_arr[i].expected_due_date ? new Date(this.external_service_provider_list_arr[i].expected_due_date) : null,
    ...
    reset_list() {
      ...
      window.axios.get(uri).then((response) => {
        this.rows = ref(response.data.storage_lot_list);
      });
    ...
    change_floor() {
      ...
      window.axios.get(uri).then((response) => {
        ...
        if (response.data.success == 100) {
          showToast('This Floor has been Saved Already.Please Select Another One', 'warning');
          return;
        }
        this.form.subrooms_list_arr = response.data.subrooms_list_arr;
        this.storage_type_arr = response.data.storage_type_arr;
    ...
    addNewCustomField() {
      if (!this.form.custom_field_name) {
        showToast('Please select Custom Field Name', 'warning');
        return;
      }
