<template>
  ...
  <fieldset>
    <form id="msform" @submit.prevent="editmode ? updateBikelot() : createCompanyProfile()"
      ...
      <div class="form-card">
        ...
        <div v-if="list_showable" class="card-body">
          ...
          <vue3-datatable :rows="rows" :columns="columns" :sortable="true" class="advanced-table whitespace-nowrap">
            ...
            <template #bike_lot_size_qty="data">
              <span class="font-semibold">{{ data.value.bike_lot_size_qty }}</span>
            </template>
            <template #buttons="data">
              <button class="btn btn-primary btn-sm" @click.prevent="editBikeLot(data.value.id)">Edit</button>
              <button class="btn btn-danger btn-sm" @click.prevent="BikeLotDelete(data.value.id)">Delete</button>
            </template>
          </vue3-datatable>
        </div>
        <div v-if="!list_showable">
                  ...
                  <div class="form-box-outer">
                    ...
                    <div class="form-holder">
                      <div class="form-check-inline">
                        ...
                        <label for="bike_lot_size_qty">Bike Lot Size:{{ form.bike_lot_size_qty }}
                          {{ uom_arr[form.lot_uom] }}</label><br>
                        <input type="radio"
                               id="bike_level_qty"
                               name="bike_level_qty" disabled checked>
                        <label for="bike_level_qty">Total Bike Level Size: {{ form.total_bike_level_size }}
                          {{ uom_arr[form.lot_uom] }}</label><br>
                        <input type="radio"
                               id="bike_stall_qty"
                               name="bike_stall_qty" disabled checked>
                        <label for="bike_stall_qty">Total Bike Stall Size: {{ form.total_bike_stall_size }}
                          {{ uom_arr[form.lot_uom] }}</label><br>
                      </div>
                    </div>
                      ...
                      <template v-if="form.total_bike_lot>0">
                        <template v-for="(single_level,level) in form.subrooms_list_arr">
                          ...
                          <tr style="background-color: #6090bc;color: #fff;">
                            <td width="" scope="row" style=""><strong>Bike Level
                          ...
                          <template v-for="(single_stall,stl) in form.subrooms_list_arr[level]">
                            <tr v-if="stl>=0">
                              <td width="" scope="row" style="padding-left:1% !important">
                                <strong>{{ single_stall.item_name }}</strong></td>
                              <td width="">
                              ...
                              <td>
                                <button type="button" class="btn btn-primary btn-sm" style="line-height:.7 !important"
                                        @click="add_stall_details(single_stall.details,level,stl)">Details
                                </button>
          ...
          <div class="text-center" v-if="!list_showable">
            ...
            <button :disabled="form.busy || editmode==false || form.posting_status!=0" type="button"
                    class="btn  btn-primary" style="min-width:110px" @click.prevent="deleteCommercialUnit()">Delete
            </button>
      ...
      <!--  MOdel  -->
      <div class="modal fade model-middle" id="BikeStallModal" tabindex="-1" role="dialog"
           aria-labelledby="exampleModalLabel" z-index="1000" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-body" style="height: 50vh; overflow-y: auto;">
              <div class="form-holder">
                <div class="row align-self-stretch">
                  <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="form-box-outer">
                      <h6><strong>Bike Level {{ details_level.padStart(2, "0") }}; Bike
                        Stall-{{ details_stall.padStart(4, "0") }}</strong></h6><br/>
                      <table class="table table_narrow">
...
export default {
  ...
  data() {
    return {
      ...
      columns: ref([
        ...
        {title: 'Lot Size', field: 'bike_lot_size_qty'},
  ...
  methods: {
    ...
    reset_list() {
      ...
      window.axios.get(uri).then((response) => {
        this.rows = ref(response.data.bike_lot_list);
      });
