<template>
  ...
  <fieldset>
    <form id="msform" @submit.prevent="editmode ? updateMailRoom() : createMailRoom()"
      ...
      <div class="form-card">
        ...
        <div v-if="list_showable" class="card-body">
          ...
          <vue3-datatable :rows="rows" :columns="columns" :sortable="true" class="advanced-table whitespace-nowrap">
            ...
            <template #room_no="data">
              <span class="font-semibold">{{ data.value.room_no }}</span>
            </template>
            <template #room_name="data">
              <span class="font-semibold">{{ data.value.room_name }}</span>
            </template>
            <template #room_uom_string="data">
              <span class="font-semibold">{{ data.value.room_uom_string }}</span>
            </template>
            <template #mailroom_size_qty="data">
              <span class="font-semibold">{{ data.value.mailroom_size_qty }}</span>
            </template>
            <template #buttons="data">
              <button class="btn btn-primary btn-sm" @click.prevent="editMailRoom(data.value.id)">Edit</button>
              <button class="btn btn-danger btn-sm" @click.prevent="MailRoomDelete(data.value.id)">Delete</button>
            </template>
          </vue3-datatable>
        </div>
        <div v-if="!list_showable">
                ...
                <div class="col-md-3 col-sm-6 col-xs-12">
                  <div class="form-box-outer">
                    ...
                    <select v-model="form.room_no"
                      ...
                      <option v-for="room in form.total_mailroom" :value="room">Room No
                        {{ form.floor_name_string }}-{{ room.toString().padStart(2, "0") }}
                      </option>
                    </select>
                      ...
                      <div class="col-md-12 col-sm-6 col-xs-12">
                        ...
                        <div class="form-holder">
                          <div class="form-check-inline">
                            ...
                            <label for="total_mailroom_size">Mail Room Size: {{ form.mailroom_size_qty }}
                              {{ uom_arr[form.room_uom] }}</label><br>

                            <input type="radio"
                                   id="total_mailbox_size"
                                   name="total_mailbox_size" disabled>
                            <label for="total_mailbox_size">Total Mail Box Size: {{ form.total_mailbox_size }}
                              {{ uom_arr[form.room_uom] }} </label><br>
          ...
          <div class="form-folder">
            <h3><i class="fa fa-hand-point-right"></i>Mail Room Details
          ...
          <div class="text-center" v-if="!list_showable">
            ...
            <button :disabled="form.busy || editmode==false || form.posting_status!=0" type="button"
                    class="btn  btn-primary" style="min-width:110px" @click.prevent="deleteMailRoom()">Delete
            </button>
...
export default {
  ...
  data() {
    return {
      ...
      columns: ref([
        ...
        {title: 'UOM', field: 'room_uom_string'},
        {title: 'Room Size', field: 'mailroom_size_qty'},
  ...
  methods: {
    ...
    reset_list() {
      ...
      window.axios.get(uri).then((response) => {
        this.rows = ref(response.data.mail_room_list);
      });
