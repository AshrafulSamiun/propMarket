      ...
      <div class="form-card">
        ...
        <div v-if="list_showable" class="card-body">
          ...
          <vue3-datatable :rows="rows" :columns="columns" :sortable="true" class="advanced-table whitespace-nowrap">
            ...
            <template #total_size_qty="data">
              <span class="font-semibold">{{ data.value.total_size_qty }}</span>
            </template>
            <template #buttons="data">
              <button class="btn btn-primary btn-sm" @click.prevent="editCommonArea(data.value.id)">Edit</button>
              <button class="btn btn-danger btn-sm" @click.prevent="CommonAreaDelete(data.value.id)">Delete</button>
            </template>
          </vue3-datatable>
        </div>
        <div v-if="!list_showable">
          ...
          <div class="form-folder">
            <h3><i class="fa fa-hand-point-right"></i>Property (Common Area) Details
                    ...
                    <table class="table table_narrow">
                      ...
                      <template v-if="form.total_common_area>0">
                        ...
                        <template v-for="(single_subroom,index) in form.subrooms_list_arr">
                          ...
                          <tr>
                            <td width="12%" scope="row" style="padding-left:1% !important">
          ...
          <div class="text-center" v-if="!list_showable">
            ...
            <button :disabled="form.busy || editmode==false || form.posting_status!=0" type="button"
                    class="btn  btn-primary" style="min-width:110px" @click.prevent="deleteCommonArea()">Delete
            </button>
...
export default {
  ...
  data() {
    return {
      ...
      columns: ref([
        ...
        {title: 'Total Area Size', field: 'total_size_qty'},
  ...
  methods: {
    ...
    reset_list() {
      ...
      window.axios.get(uri).then((response) => {
        this.rows = ref(response.data.common_area_list);
      });
