<template>
  <fieldset>
    <form id="msform" @submit.prevent="editmode ? updateParkingLot() : createCompanyProfile()" @keydown="form.onKeydown($event)">
      <div class="form-card">
        ...
        <div v-if="list_showable" class="card-body">
          ...
          <vue3-datatable :rows="rows" :columns="columns" :sortable="true" :search="filter" class="advanced-table whitespace-nowrap">
            ...
            <template #parking_lot_size_qty="data">
              <span class="font-semibold">{{ data.value.parking_lot_size_qty }}</span>
            </template>
            <template #buttons="data">
              <button class="btn btn-primary btn-sm" @click.prevent="editParkingLot(data.value.id)">Edit</button>
              <button class="btn btn-danger btn-sm" @click.prevent="ParkingLotDelete(data.value.id)">Delete</button>
            </template>
          </vue3-datatable>
        </div>
                  ...
                  <div class="form-box-outer">
                    ...
                    <div class="form-holder">
                      <div class="form-check-inline">
                        ...
                        <label for="parking_lot_size_qty">Parking Lot Size:{{ form.parking_lot_size_qty }}
                          {{ uom_arr[form.lot_uom] }}</label><br>
                        <input type="radio"
                               id="parking_level_qty"
                               name="parking_level_qty" disabled checked>
                        <label for="parking_level_qty">Total Parking Level Size: {{ form.total_parking_level_size }}
                          {{ uom_arr[form.lot_uom] }}</label><br>
                        <input type="radio"
                               id="parking_stall_qty"
                               name="parking_stall_qty" disabled checked>
                        <label for="parking_stall_qty">Total Parking Stall Size: {{ form.total_parking_stall_size }}
                          {{ uom_arr[form.lot_uom] }}</label><br>
                      </div>
                    </div>
                      ...
                      <template v-if="form.total_parking_lot>0">
                        <template v-for="(single_level,level) in form.subrooms_list_arr">
                          ...
                          <tr style="background-color: #6090bc;color: #fff;">
                            <td width="" scope="row" style=""><strong>Parking Level
      ...
      <!--  MOdel  -->
      <div class="modal fade model-middle" id="ParkingStallModal" tabindex="-1" role="dialog"
           aria-labelledby="exampleModalLabel" z-index="1000" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content" style="width:800px;overflow-y: initial !important">
            ...
            <div class="modal-body" style="height: 50vh; overflow-y: auto;">
              <div class="form-holder">
                <h6><strong>Parking Level {{ details_level.padStart(2, "0") }}; Parking
...
export default {
  ...
  data() {
    return {
      ...
      columns: ref([
        ...
        {title: 'Lot Size', field: 'parking_lot_size_qty'},
  ...
  methods: {
    ...
    reset_list() {
      ...
      window.axios.get(uri).then((response) => {
        this.rows = ref(response.data.parking_lot_list);
      });
