  ...
  methods: {
    ...
    fetchRulesPolicyUpdate(id) {
      ...
      window.axios.get(uri).then((response) => {
        ...
        this.form.issued_date = response.data.rules_policy_data[0].issued_date && response.data.rules_policy_data[0].issued_date !== "" ?
            new Date(response.data.rules_policy_data[0].issued_datee) : null;
